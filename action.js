...
async function run() {
    ...
    latestGoVersion = await fixLatestVersion(currentGoModVersion, latestGoVersion, false)
...
// Example: 1.23.0 -> 1.23
async function fixLatestVersion(currentGoModVersion, latestGoVersion, force) {
  if (focrce || currentGoModVersion.split('.').length == 2) {
...
async function createPullRequest(githubToken, branchName, repoOwner, repoName, currentGoVersion, latestGoVersion) {
  ...
  latestGoVersionForReleaseNotes = await fixLatestVersion(currentGoModVersion, latestGoVersion, true)

  let title = `Bump Go from ${currentGoVersion} to ${latestGoVersion}`
  let body = `Bump Go from ${currentGoVersion} to ${latestGoVersion}\n\nRelease notes Go ${latestGoVersion} of can be found here:\n\nhttps://go.dev/doc/go${latestGoVersionForReleaseNotes}`
